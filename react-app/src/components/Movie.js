...
class Movies extends Component {
        ...
        if (chk.islogin == false) {
            ...
            fetch(request)
                ...
                .then(data => {
                    ...
                    this.setState({ data: this.state.movies })
                    this.setState({ val: this.state.movies[0] })
                });
        }
    }
    handleCChange(event) {
    ...
    handleApply(event) {
        ...
        fetch('http://localhost:8080/cat/Movies/' + this.state.val)
            .then(response => response.json())
            .then(data => {
                // console.log(data);
        ...
        var chk = JSON.parse(window.localStorage.getItem('chk'));
        fetch('http://localhost:8080/userquiz/' + chk.uname + '/Movies' + '/' + this.state.val)
            .then(response => response.json())
            .then(data => {
    ...
    handleSubmitquiz(event) {
        ...
        fetch(request)
            .then(response => response.json())
            .then(data => {
                data.moviesscore = String(Number(data.moviesscore) + this.state.score)
                ...
                fetch('http://localhost:8080/userquiz', {
                    method: 'POST',
                    body: JSON.stringify({ "username": chk.uname, "category": "Movies", "quizno": String(this.state.val), "score": String(this.state.score) }),
                })
            });
        this.setState({ quizdone: true })
    }
