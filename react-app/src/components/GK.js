...
class GK extends Component {
        ...
        if (chk.islogin == false) {
            ...
            fetch(request)
                ...
                .then(data => {
                    ...
                    // console.log(this.state.gk)
                    this.setState({ data: this.state.gk })
                    this.setState({ val: this.state.gk[0] })
                });
        }
    }
    handleCChange(event) {
    ...
    handleApply(event) {
        ...
        fetch('http://localhost:8080/cat/General Knowledge/' + this.state.val)
            .then(response => response.json())
            .then(data => {
                // console.log(data);
        ...
        var chk = JSON.parse(window.localStorage.getItem('chk'));
        fetch('http://localhost:8080/userquiz/' + chk.uname + '/General Knowledge' + '/' + this.state.val)
            .then(response => response.json())
            .then(data => {
    ...
    handleSubmitquiz(event) {
        ...
        fetch(request)
            .then(response => response.json())
            .then(data => {
                data.gkscore = String(Number(data.gkscore) + this.state.score)
                ...
                fetch('http://localhost:8080/userquiz', {
                    method: 'POST',
                    body: JSON.stringify({ "username": chk.uname, "category": "General Knowledge", "quizno": String(this.state.val), "score": String(this.state.score) }),
                })
            });
        this.setState({ quizdone: true })
    }
